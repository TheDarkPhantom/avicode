    ...
    <>
      {!isRepo ? (
        ...
        <div className="flex shrink-0 items-center gap-1.5">
          {allFiles.length > 0 && onOpenChanges ? (
            <Tooltip>
              <TooltipTrigger
                render={
                  <Button
                    aria-label={`${allFiles.length} changed ${
              ...
              >
                ...
                <span className="tabular-nums">{allFiles.length}</span>
              </TooltipTrigger>
              <TooltipPopup side="bottom">
                {allFiles.length} changed {allFiles.length === 1 ? "file" : "files"}
              </TooltipPopup>
            </Tooltip>
          ) : null}
