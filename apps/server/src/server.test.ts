    ...
    const servedRoutesLayer = HttpRouter.serve(makeRoutesLayer, {
      ...
      Layer.provide(
        Layer.mergeAll(
          ...
          // Avi Code addition: the `/btw` RPC handler resolves ProviderService,
          // so the router now depends on it. Merged in here rather than as
          // another `Layer.provide` — this pipe is already at `pipe`'s 20-arg
          // overload ceiling.
...
it.layer(NodeServices.layer)("server router seam", (it) => {
  ...
  it.effect("resumes an already-created bootstrap thread without creating it twice", () =>
    Effect.gen(function* () {
      const dispatchedCommands: Array<OrchestrationCommand> = [];
      const createWorktree = vi.fn(() =>
        Effect.succeed({
          worktree: { refName: "t3code/duplicate", path: "/tmp/duplicate-worktree" },
        }),
      );
      ...
      const response = yield* Effect.scoped(
        withWsRpcClient(wsUrl, (client) =>
          client[ORCHESTRATION_WS_METHODS.dispatchCommand]({
            type: "thread.turn.start",
            commandId: CommandId.make("cmd-retry-bootstrap-turn-start"),
            threadId: ThreadId.make("thread-existing-bootstrap"),
            message: {
